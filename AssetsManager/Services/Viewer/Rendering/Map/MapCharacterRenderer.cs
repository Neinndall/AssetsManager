...
namespace AssetsManager.Services.Viewer.Rendering.Map
{
    ...
    internal sealed class MapCharacterRenderer : IDisposable
    {
        ...
        internal sealed record RenderGroup(
            ...
            Vector3? erroredLinear = null)
        {
            ...
            BuildQueues(groups, cameraPosition, timeSeconds);
        ...
        private void BuildQueues(IReadOnlyList<RenderGroup> groups, Vector3 cameraPosition, float timeSeconds)
        {
            _opaque.Clear();
            _transparent.Clear();
            ...
            foreach (RenderGroup group in groups)
            {
                ...
                foreach (IGrouping<string, MapCharacterData> animationGroup in group.Placements
                             .Where(placement => placement != null)
                             .GroupBy(placement => placement.Animation ?? string.Empty, StringComparer.OrdinalIgnoreCase))
                {
                    Matrix4x4[] joints = group.Animation.Evaluate(group.Asset, animationGroup.Key, timeSeconds);
                    Matrix4x4[] palette = GetPalette(resources, animationGroup.Key);
                    FillInfluencePalette(palette, joints, resources.InfluenceJointSlots);

                    foreach (MapCharacterData placement in animationGroup)
                    {
                        Matrix4x4 world = CreateWorldMatrix(placement.Transform, group.Asset.Skin.Scale);
                        float distance = Vector3.DistanceSquared(Vector3.Transform(Vector3.Zero, world), cameraPosition);
        ...
        {
            Matrix4x4 placement = MapCharacterSemantics.SceneTransform(authoredTransform);
